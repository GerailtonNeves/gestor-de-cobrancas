...
app.post('/api/testar-alerta-admin', async (req, res) => {
  try {
    ...
    if (waStatus !== 'CONNECTED' || !waSock) {
      return res.status(400).json({ error: "O WhatsApp não está conectado no sistema. Conecte pelo QR Code na aba 'Conectar WhatsApp (QR)' antes de testar." });
    }
    ...
    return res.json({ success: true, message: `Mensagem de teste disparada com sucesso para ${config.whatsappAdmin}!` });
  } catch (err) {
    console.error("❌ Erro em POST /api/testar-alerta-admin:", err);
